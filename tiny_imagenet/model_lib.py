...
def default_hparams():
  ...
  optimizer = 'adam'  # possible values are: 'rms', 'momentum', 'adam'

  if optimizer == 'rms':
    lr = 0.045
    lr_list = [1.0 / 6, 2.0 / 6, 3.0 / 6, 4.0 / 6, 5.0 / 6, 1.0, 0.1, 0.01, 0.001, 0.0001]
    lr_decay_epochs = [1, 2, 3, 4, 5, 30, 60, 80, 90]
  elif optimizer == 'adam':
    lr = 0.001
    lr_list = [lr, lr/10, lr/100]
  ...
  return tf.contrib.training.HParams(
      ...
      lr_schedule='step',  # Possible values: 'exp_decay', 'step', 'fixed'
...
def get_lr_schedule(hparams, examples_per_epoch, replicas_to_aggregate=1):
  ...
  # learning_rate *= batch_size // 32  # we increase the learning rate as much as batch size is larger than 32
  # if replicas_to_aggregate > 0:
    ...
    def resnet_model(images, is_training, reuse=tf.AUTO_REUSE):
      with tf.contrib.framework.arg_scope(resnet_v2.resnet_arg_scope()):
        ...
        logits, _ = resnet_fn(images, num_classes, is_training=is_training,
                              reuse=reuse)
        logits = tf.reshape(logits, [-1, num_classes])  # from [bs x 1 x 1 x num_classes] to [bs x num_classes]
      return logits
    return resnet_model
  else:
