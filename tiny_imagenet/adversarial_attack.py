...
https://alp_github.com/tensorflow/cleverhans
"""
...
def generate_pgd_rand_multrest(x, y_ground_truth, bounds, model_fn, attack_params):
  # pylint: disable=g-doc-args
  """Generats targeted PGD adversarial examples with random target class.
  ...
  logits = model_fn(x)
  num_classes = tf.shape(logits)[1]
  one_hot_labels = tf.one_hot(y_ground_truth, num_classes)

  return generate_pgd_common(x, bounds, model_fn, attack_params,
                             one_hot_labels=one_hot_labels,
                             perturbation_multiplier=-1.0)

def generate_pgd(x, y_ground_truth, bounds, model_fn, attack_params):
  # pylint: disable=g-doc-args
...
def generate_adversarial_examples(x, y_ground_truth, bounds, model_fn, attack_description, n_restarts=1):
  ...
  if attack_name == 'pgdll':
    ...
    if n_restarts == 1:
      ...
      return generate_pgd_rand_multrest(x, y_ground_truth, bounds, model_fn, attack_params)
  elif attack_name == 'pgd':
    return generate_pgd(x, y_ground_truth, bounds, model_fn, attack_params)
  else:
    raise ValueError('Invalid value of attack description %s'
                     % attack_description)
